...
contract BettingContract is Ownable {
    ...
    mapping(address => bool) public obtainedSoap;
 ...
 // Distribute is useless, unless someone sends BNB directly to the contract, which is not meant to happen
    ...
    function calculateWinnings(address user, uint team, uint256 round) public view returns(uint256) {
        uint256 winnings = amountUserPutIn[round][user].mul(totalBets[round]).div(totalBetOnTeam[round][team]);
        return winnings;
    }

    function placeBet(uint256 amount, uint256 teamToBetOn) public {  
        require(betsOpen == true, "you have to wait for bets to be open"); 
        require(amount > 0, "you must input an amount to bet");     
        uint256 amountToBet = amount;
        ...
        if (roundUserBettedOn[msg.sender] == currentRound) {
            if (teamToBetOn != teamUserBettedOn[currentRound][msg.sender]) {
                ...
                totalBetOnTeam[currentRound][teamBettedOn()] = totalBetOnTeam[currentRound][teamBettedOn()].sub(previousAmount);
        ...
        soapContr.transferFrom(msg.sender, address(this), amountToBet.mul(MULTIPLIER));
    ...
    function takeOutBet(uint256 amount) public {
        ...
        soapContr.transferFrom(address(this), owner(), welchersFee.mul(MULTIPLIER));
        soapContr.transferFrom(address(this), msg.sender, amountToTakeOut.mul(MULTIPLIER));
    ...
    function pullBetWithFailsafe() public {
        ...
        soapContr.transferFrom(address(this), msg.sender, amountUserPutIn[currentRound][msg.sender].mul(MULTIPLIER));
    ...
    function declareWinner(uint winningTeam) public onlyReferee() {
        isWinner[currentRound][winningTeam] = true;
        roundWinner[currentRound] = winningTeam;
        winnerDeclared[currentRound] = true;
    }

    function getSoap() public { //this function is only for test purposes
        require(obtainedSoap[msg.sender] == false, "You have already obtained SOAP");
        soapContr.transferFrom(address(this), msg.sender, 10000000 * MULTIPLIER);
        obtainedSoap[msg.sender] = true;
    }

    function withdrawWinnings(uint256 round) public {
        // Calculate winnings and take out value from pot.
        require(winnerDeclared[round] == true, "Winner has not yet been declared");
        ...
        if(winnings > 0) {
            if(changedBet[round][msg.sender] == true) {
                ...
                soapContr.transferFrom(address(this), owner(), changingFee.mul(MULTIPLIER));            
            }
            soapContr.transferFrom(address(this), msg.sender, winnings.mul(MULTIPLIER));
        }
        winningsWithdrawn[round][msg.sender] = true;
    }

    function amountBetOnTeam(uint team, uint256 round) public view returns (uint256) {
        return totalBetOnTeam[round][team];
    }

    function teamBettedOn() public view returns (uint256) {
        return teamUserBettedOn[currentRound][msg.sender];
    }

    function addBets(uint256 team, uint256 amount) internal {
        amountUserPutIn[currentRound][msg.sender] = amountUserPutIn[currentRound][msg.sender].add(amount);
        totalBetOnTeam[currentRound][team] = totalBetOnTeam[currentRound][team].add(amount);
        totalBets[currentRound] = totalBets[currentRound].add(amount);
    }
    ...
    function approveSoapSpending() public {
        soapContr.approveMax(address(this));
    }
